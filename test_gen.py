...
class DCGAN(object):
    ...
    def __init__(self,*L):
        ...
        self.stack.compile(loss=custom_objective(self.bias), optimizer=optimizerG, metrics=['accuracy'])
    ...
    def train(self,totalEpochs, batchSize,save_interval,callback):
        ...
        for epoch in tqdm(range(totalEpochs)):
            ...
            # Smooth Labelling 
            yDis = np.zeros([2 * self.batchSize, 1])
            yDis[:batchSize, :] = 0.9
            ...
            if epoch % save_interval:
                plot_images(self.generator,epoch,noiseClass,noiseInput,0,self.GLR,self.DLR,self.runNo,self.bias)
...
def plot_images(generator,epoch,noiseClass,noiseInput,isTest,GLR,DLR,runNo,bias):
...
def train_network(GLR,DLR,runNo,bias):
    ...
    if args.generator:
        ...
        L = [GLR,DLR,runNo,bias]
        gan = DCGAN(*L)

        callback = TensorBoard(gan.NAME)
        callback.set_model(gan.stack)

        gan.train(2000, gan.batchSize, 100, callback)
