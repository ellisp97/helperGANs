...
class Generator:
    def __init__(self, img_shape, batch_size):
        ...
        with tf.variable_scope('g'):
            print("Initialize generator weights")
    ...
    # Activation Function = Leaky RELU, applied to output layer of neural network
    # given as output from batch normalisation
    def forward(self, X, momentum=0.5):
        z = tf.matmul(X, self.W1)
        z = tf.nn.relu(z)
        z = tf.reshape(z, [-1, 7, 7, 512])

        # regulaizer function if the network contains exatcly one fully connectedlayer
        # if not must use some other max-pooling method
        z = UpSampling2D()(z)
