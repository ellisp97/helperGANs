...
class DCGAN(object):
    ...
    def __init__(self,*L):
        ...
        self.NAME = "new_logs_test/MNIST-G:{}-D:{}-R:{}".format(self.GLR,self.DLR,self.runNo)#,int(time.time()))
...
def plot_images(generator,epoch,noiseClass,noiseInput,isTest,GLR,DLR,runNo):
    ...
    if isTest:
        NAME = "new_logs_test/TEST-MNIST-G:{}-D:{}-R:{}".format(GLR,DLR,runNo)#,int(time.time()))
    else: 
        NAME = "new_logs_test/MNIST-G:{}-D:{}-R:{}".format(GLR,DLR,runNo)#,int(time.time()))
